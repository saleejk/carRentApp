...
class HomeBike extends StatefulWidget {
  const HomeBike({super.key});
  @override
  State<HomeBike> createState() => _HomeBikeState();
}

class _HomeBikeState extends State<HomeBike> {
  String search = '';
  List<AddBikeModel> searchedList = [];
  void searchListUpdate() {
    getBikeDetails();
    searchedList = bikeListNotifier.value
        .where((bike) =>
            bike.bikeName.toLowerCase().contains(search.toLowerCase()))
        .toList();
  }
    ...
    return Scaffold(
      ...
      Expanded(
          child: ValueListenableBuilder(
              ...
              builder: (BuildContext ctx, List<AddBikeModel> newbikeListt,
                  Widget? child) {
                if (search.isNotEmpty) {
                  if (newbikeListt.isEmpty) {
                    return SizedBox(
                        child: Center(
                            child: Lottie.asset(
                    ...
                    return bikeListbuild(searchedList);
                  }
                } else {
                  return bikeListbuild(newbikeListt);
                }
              }))
    ]));
  }
